				...
				{isLoading ? (
					...
					<ResponsiveContainer width="100%" height="100%">
						<AreaChart data={data} baseValue="dataMin" margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
							...
							<Area type="monotone" dataKey="value" stroke="#00FF88" fill="#00FF88" fillOpacity={0.08} strokeWidth={2} />
						</AreaChart>
					</ResponsiveContainer>
				)}
			</div>
...
function MonthlySplitCard({ title, unit, clean, fossil }: { title: string; unit: string; clean: string; fossil: string }) {
	return (
		<div className="rounded-2xl border border-slate-800 bg-gradient-to-br from-[#111827] to-[#0f172a] p-5 shadow-xl shadow-black/20">
			<div className="mb-4 flex items-center justify-between">
				<h3 className="text-base font-semibold text-white">{title}</h3>
				<span className="rounded-full border border-slate-700 bg-slate-800 px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.18em] text-slate-300">{unit}</span>
			</div>
			<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
				<div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4">
					<div className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-300">Clean</div>
					<div className="mt-3 text-3xl font-bold tracking-tight text-white">{clean}</div>
				</div>
				<div className="rounded-xl border border-amber-500/20 bg-amber-500/10 p-4">
					<div className="text-xs font-semibold uppercase tracking-[0.18em] text-amber-300">Fossil</div>
					<div className="mt-3 text-3xl font-bold tracking-tight text-white">{fossil}</div>
				</div>
			</div>
		</div>
	);
			...
			<div className="mx-auto max-w-7xl space-y-6">
								...
								<motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
									...
									{filteredCountries.length > 0 ? filteredCountries.map((c) => (
										<div key={c.name} onClick={() => handleSelectCountry(c)}
										     className={`flex cursor-pointer items-center justify-between px-4 py-2.5 hover:bg-[#374151] ${selectedCountry?.name === c.name ? "bg-[#374151]/50" : ""}`}>
											<div className="flex items-center gap-2"><span className="text-xl">{c.flag}</span>
												<span className="text-sm font-medium">{highlightMatch(c.name, searchQuery)}</span></div>
											{selectedCountry?.name === c.name && <Check className="h-4 w-4 text-[#00FF88]" />}
										</div>
				...
				{!hideTopStats && (
					<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
						<StatCard title="GDP per Energy Use" value={statValues.gdp} icon={<Activity className="h-5 w-5 text-blue-400" />} isLoading={isLoading} />
						<StatCard title="Energy Import Reliance" value={statValues.import} icon={<Box className="h-5 w-5 text-amber-400" />} isLoading={isLoading} />
						<StatCard title="Total Population" value={statValues.pop} icon={<MapPin className="h-5 w-5 text-purple-400" />} isLoading={isLoading} />
					</div>
				)}
				...
				{showSingleMonthCards && baselineRow && (
					<motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
					            className="flex flex-col items-start justify-between gap-3 rounded-xl border border-[#00FF88]/30 bg-[#00FF88]/5 px-5 py-4 sm:flex-row sm:items-center">
						<div>
							<p className="text-sm font-semibold text-[#00FF88]">📌 Ready to simulate</p>
						...
						<button onClick={handleUseAsBaseline} disabled={baselineSet}
						        className="flex items-center gap-2 rounded-lg bg-[#00FF88] px-4 py-2 text-sm font-bold text-slate-900 transition-all hover:bg-[#00e07a] disabled:opacity-60">
