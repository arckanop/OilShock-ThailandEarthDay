...
function MetricTrendCard({title, data, suffix, isLoading}: {
	title: string;
	data: TrendPoint[];
	suffix: string;
	isLoading: boolean
		...
		<div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
			...
			<div className="h-72">
				{isLoading ? (
					...
					<div className="flex h-full items-center justify-center text-sm text-slate-400">No data
						available</div>
				) : (
					<ResponsiveContainer width="100%" height="100%">
						<AreaChart data={data} baseValue="dataMin" margin={{top: 8, right: 8, left: 0, bottom: 0}}>
							...
							<YAxis stroke="#94a3b8" tickLine={false} axisLine={false} width={56} tickCount={6}
							       allowDecimals={true}
							       tick={{fontSize: 12, fill: "#64748b"}}
							       domain={([dataMin, dataMax]: readonly [number, number]) => {
								       if (dataMin === dataMax) return [dataMin - 1, dataMax + 1] as [number, number];
							...
							/>
							<Tooltip
								formatter={(value) => Number(value ?? 0).toFixed(2).replace(/\.?0+$/, "")}
								contentStyle={{
									backgroundColor: "#fff",
							...
							<Area type="monotone" dataKey="value" stroke="#10b981" fill="#10b981" fillOpacity={0.08}
							      strokeWidth={2}/>
						</AreaChart>
					</ResponsiveContainer>
				)}
...
function MonthlySplitCard({title, unit, clean, fossil}: {
	title: string;
	unit: string;
	clean: string;
	fossil: string
	...
	return (
		<div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
			<div className="mb-4 flex items-center justify-between">
				...
				<span
					className="rounded-full border border-slate-200 bg-slate-100 px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.18em] text-slate-500">{unit}</span>
			</div>
...
const RealData = () => {
	...
	useEffect(() => {
		...
		if (year === "all") {
			...
			setCapacityYearlyData(summary.map((r) => ({
				name: String(r.year),
				clean: Number(r.capClean.toFixed(2)),
				fossil: Number(r.capFossil.toFixed(2))
			})));
			setGenerationYearlyData(summary.map((r) => ({
				name: String(r.year),
				clean: Number(r.genClean.toFixed(2)),
				fossil: Number(r.genFossil.toFixed(2))
			})));
			setEmissionsYearlyData(summary.map((r) => ({
				name: String(r.year),
				clean: Number(r.emitClean.toFixed(2)),
				fossil: Number(r.emitFossil.toFixed(2))
			})));
			setStatValues({
				gdp: latestGdp.toFixed(2) + " $/TBTU",
				import: latestImport.toFixed(2) + " TWh",
				pop: latestPop.toFixed(2) + " M"
			});
			setCapacityData([]);
			setGenerationData([]);
			setEmissionsData([]);
		...
		<div className="space-y-6">
			<div className="mb-8">
				...
				<p className="mt-1 text-slate-500">
					Explore Ember monthly data (2020–2024) across 72 countries — send a specific month to the Scenario
					Simulator.
				</p>
			</div>
			...
			{/* Filter bar */}
			<div
				className="grid grid-cols-1 items-center gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-12">
				<div className="relative md:col-span-7 lg:col-span-5" ref={searchRef}>
					<label
						className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">Location</label>
					<div className="relative">
						<div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3"><Search
						...
						<div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3"><span
							className="text-lg">{selectedCountry?.flag ?? "🌍"}</span></div>
					</div>
					<AnimatePresence>
						{isDropdownOpen && (
								...
								{filteredCountries.length > 0 ? filteredCountries.map((c) => (
									<div key={c.name} onClick={() => handleSelectCountry(c)}
										...
										<div className="flex items-center gap-2"><span
											className="text-xl">{c.flag}</span>
											<span
												className="text-sm font-medium text-slate-900">{highlightMatch(c.name, searchQuery)}</span>
										</div>
				...
				<div className="md:col-span-5 lg:col-span-3">
					<label
						className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">Year</label>
					<div className="relative">
						...
						<div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
							<ChevronDown className="h-4 w-4 text-slate-400"/></div>
					</div>
				</div>
			</div>
			...
			{!hideTopStats && (
				<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
					<StatCard title="GDP per Energy Use" value={statValues.gdp}
					          icon={<Activity className="h-5 w-5 text-blue-500"/>} isLoading={isLoading}/>
					<StatCard title="Energy Import Reliance" value={statValues.import}
					          icon={<Box className="h-5 w-5 text-amber-500"/>} isLoading={isLoading}/>
					<StatCard title="Total Population" value={statValues.pop}
					          icon={<MapPin className="h-5 w-5 text-purple-500"/>} isLoading={isLoading}/>
				</div>
			)}
				...
				<div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
					<div className="grid grid-cols-1 gap-4 md:grid-cols-12 md:items-center">
						<div className="md:col-span-4 lg:col-span-3">
							<label
								className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">Month</label>
							<div className="relative">
								<select value={month}
				...
				<motion.div initial={{opacity: 0, y: 8}} animate={{opacity: 1, y: 0}}
					...
					<div>
						...
						<p className="mt-0.5 text-xs text-slate-500">
							Use {selectedCountry?.name} · {month} {year} (demand {baselineRow.demand} TWh) as the
							baseline
						</p>
					</div>
					<button onClick={handleUseAsBaseline} disabled={baselineSet}
						...
						{baselineSet ? (<><Check className="h-4 w-4"/>Sent to Simulator</>) : (<><SlidersHorizontal
							className="h-4 w-4"/>Use as Baseline</>)}
					</button>
				</motion.div>
			)}
			...
			{showAllYearsGraphs && (
				<div className="space-y-6">
					<div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
						<MetricTrendCard title="GDP per Energy Use by Year" data={gdpTrendData} suffix="$/TBTU"
						                 isLoading={isLoading}/>
						<MetricTrendCard title="Energy Import Reliance by Year" data={importTrendData} suffix="TWh"
						                 isLoading={isLoading}/>
						<MetricTrendCard title="Population by Year" data={popTrendData} suffix="M"
						                 isLoading={isLoading}/>
					</div>
					<div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
						<ChartCard title="Capacity Avg by Year (GW)" data={capacityYearlyData} isLoading={isLoading}/>
						<ChartCard title="Electricity Generation Avg by Year (TWh)" data={generationYearlyData}
						           isLoading={isLoading}/>
						<ChartCard title="Power Sector Emissions Avg by Year (mtCO2)" data={emissionsYearlyData}
						           isLoading={isLoading}/>
					</div>
				</div>
			)}
			...
			{showSingleMonthCards && (
				<div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
					<MonthlySplitCard title="Capacity" unit="GW" clean={monthlyValues.capacity.clean}
					                  fossil={monthlyValues.capacity.fossil}/>
					<MonthlySplitCard title="Electricity Generation" unit="TWh" clean={monthlyValues.generation.clean}
					                  fossil={monthlyValues.generation.fossil}/>
					<MonthlySplitCard title="Power Sector Emissions" unit="mtCO2" clean={monthlyValues.emissions.clean}
